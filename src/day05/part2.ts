...
function checkIfPageCorrect(page: number[]) {
  ...
  let elToSwap1: number;
  let elToSwap2: number;
  ...
  for (const el of page) {
    ...
    if (elementVaues) {
      ruleBroken1 = elementsBefore.some((element) => {
        const includesElement = elementVaues.includes(element);
        if (includesElement) {
          elToSwap2 = page.indexOf(element);
        }
    ...
    for (const elementAfter of elementsAfter) {
      ...
      if (elementAfterVaues) {
        ...
        if (ruleBroken2) {
          elToSwap2 = page.indexOf(elementAfter);
          break;
        }
      }
    }
  }
  if (ruleBroken1 || ruleBroken2) {
    [page[elToSwap1], page[elToSwap2]] = [page[elToSwap2], page[elToSwap1]];
    return checkIfPageCorrect(page);
  }
...
// subtract score from part 1 and this score to get result
console.log("Total score: ", findCorrectPages());
